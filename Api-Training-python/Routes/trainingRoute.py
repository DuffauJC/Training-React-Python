...
@router.get("/trainings/",response_model=List[schemas.Training])
async def get_all_trainings(session:Session=Depends(get_db)):
    return await trainingController.read_trainings(session)

@router.get("/trainingById/{training_id}",response_model=schemas.Training)
...
@router.post("/trainingCreate/",response_model=schemas.Training)
...
@router.put("/trainingUpdate/{training_id}",response_model=schemas.Training)
...
@router.delete("/trainingDelete/{training_id}")
